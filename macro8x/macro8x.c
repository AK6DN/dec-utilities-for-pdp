...
/*    v3.2  18Jan08  DNN  Bug fixes and updates:                              */
/*                        - Added EJECT pseudo-op                             */
/*                        - Added symbols KIE,TFL,TSK to symbol table         */
/*                        - Added ASCII/ASCIZ pseudo-ops                      */
/*                        - Changed LINELN from 96 to 250                     */
/*    v3.3  23Jan08  DNN  Added missing MQ instr CAM,SWP,ACL                  */
/*    v3.4  23Jan09  DNN  Added memory map usage display at end               */
/*    v3.5  20Feb13  DNN  Added DEVICE pseudo-op                              */
/*    v3.51 06May15  DNN  Added 'iy >= 2' test at end of readline for CR      */
/*                          present check to guarantee iy-2 is >= zero always */
/*                        Added code to resolve all -Wall error messages:     */
/*                          Added (unsigned) to arguments to printf for %o.   */
...
// resolves signed char subscript warning in gcc
#define is_alpha(c) (isalpha((unsigned)(c)))
#define is_digit(c) (isdigit((unsigned)(c)))
#define is_alnum(c) (isalnum((unsigned)(c)))
#define is_lower(c) (islower((unsigned)(c)))
#define is_upper(c) (isupper((unsigned)(c)))
#define to_lower(c) (tolower((unsigned)(c)))
#define to_upper(c) (toupper((unsigned)(c)))

#define LINELEN             250         /* max line length, was 96            */
...
#define LEADER_LENGTH        16
#define TRAILER_LENGTH       16
...
enum pseudo_t
{
  ASCII,  ASCIZ,    BANK,     BINPUNCH, DECIMAL,  DEFINE, DEVICE,
  DUBL,   EJECT,    ENPUNCH,  EXPUNGE,  FIELD,    FIXTAB, FLTG,
  IFDEF,  IFNDEF,   IFNZERO,  IFNZRO,   IFZERO,   LGM,    LIST,
  LIT,    LITBAS,   NOLGM,    NOPUNCH,  OCTAL,    PAGE,   PAUSE,
  RELOC,  RIMPUNCH, TEXT,     TITLE,    UNLIST,   VFD,    ZBLOCK
};
...
SYM_T pseudo[] =
{
  { PSEUDO, "ASCII",  ASCII   },    /* Pack ASCII into memory.                */
  { PSEUDO, "ASCIZ",  ASCIZ   },    /* Pack ASCII into memory, null term.     */
  ...
  { PSEUDO, "DEVICE", DEVICE  },    /* Pack 1-4 ASCII characters into 2 words */
...
SYM_T permanent_symbols[] =
{
  ...
  { FIXED,  "CAM",    07621   },    /* Clear AC and MQ                        */
  { FIXED,  "SWP",    07521   },    /* Swap MQ and AC                         */
  ...
  /* Keyboard/Reader                                                          */
  { FIXED,  "KCF",    06030   },    /* CLEAR KEYBOARD FLAG                    */
  { FIXED,  "KSF",    06031   },    /* SKIP ON KEYBOARD FLAG SET              */
  { FIXED,  "KCC",    06032   },    /* CLEAR KEYBOARD FLAG & SET READER RUN   */
  { FIXED,  "KRS",    06034   },    /* READ KEYBOARD BUFFER (STATIC)          */
  { FIXED,  "KIE",    06035   },    /* SET/CLR KEYBOARD INTR ENABLE FROM AC11 */
  { FIXED,  "KRB",    06036   },    /* READ KEYBOARD BUFFER & CLEAR FLAG      */
  /* Teleprinter/Punch                                                        */
  { FIXED,  "TFL",    06040   },    /* SET TELEPRINTER FLAG                   */
  { FIXED,  "TSF",    06041   },    /* SKIP ON TELEPRINTER FLAG               */
  { FIXED,  "TCF",    06042   },    /* CLEAR TELEPRINTER FLAG                 */
  { FIXED,  "TPC",    06044   },    /* LOAD TELEPRINTER & PRINT               */
  { FIXED,  "TSK",    06045   },    /* SKIP IF TELPRINTER INTR SET            */
...
EMSG_T  literal_overflow    = { "PE page exceeded", "current page literal capacity exceeded" };
EMSG_T  pz_literal_overflow = { "ZE page exceeded", "page zero capacity exceeded" };
...
EMSG_T  illegal_field_value = { "expr out of range", "field value not in range of 0 through 7" };
EMSG_T  illegal_vfd_value   = { "width out of range", "VFD field width not in range" };
EMSG_T  no_literal_value    = { "no value",  "No literal value" };
EMSG_T  text_string         = { "no delimiter", "Text string delimiters not matched" };
EMSG_T  in_rim_mode         = { "not OK in rim mode" "FIELD pseudo-op not valid in RIM mode" };
...
EMSG_T  bad_dummy_arg       = { "bad dummy arg", "Bad dummy argument following DEFINE" };
EMSG_T  macro_too_long      = { "macro too long", "Macro too long" };
EMSG_T  no_virtual_memory   = { "out of memory", "Insufficient memory for macro" };
...
BOOL    xref;			/* Print xref table flag		      */
BOOL    memmap;			/* Print memory map table flag		      */
...
BOOL	memory_map[(FIELD_FIELD|PAGE_FIELD|ADDRESS_FIELD)+1]; /* memory map   */
WORD32	memory_max;		/* highest memory loc used		      */
WORD32	memory_min;		/* lowest memory loc used		      */
...
int main( int argc, char *argv[] )
{
  ...
  for( ix = 0; ix < sizeof(memory_map); ix++ ) memory_map[ix] = FALSE;
  memory_min = sizeof(memory_map)+1;
...
void getArgs( int argc, char *argv[] )
{
    ...
    if( argv[ix][0] == '-' )
    {
      for( jx = 1; argv[ix][jx] != 0; jx++ )
      {
        ...
        default:
          ...
          fprintf( stderr, " -u -- output memory usage map to file\n" );
          fflush( stderr );
          exit( -1 );
        } /* end switch                                                       */
      } /* end for                                                            */
  ...
  if( is_alpha( pathname[0] ) && pathname[1] == ':' && pathname[2] != '\\' )
  {
          ...
          default:
            if( is_alpha( line[lexstart] ))
            {
              ...
              {                     /* Find arguments.                       */
                ...
                for( jx = 0; !is_done( line[cc] ) && ( jx < MAC_MAX_ARGS ); cc++ )
                {
...
SYM_T *eval()
{
  ...
  else if( is_digit( line[lexstart] ))
  {
    from = lexstart;
    val = 0;
    while( from < lexterm )
    {
      if( is_digit( line[from] ))
      {
  ...
  do
  {
    while( scanning_line )
    {
      ...
      else
      {
        ...
        case '-':
        default:
          if( is_digit( line[lexstart] ) || line[lexstart] == '-' )
          {
...
FLTG_T *evalFltg()
{
  ...
  while( TRUE )
  {
    ...
    default:
      if( is_digit( line[lexstart] ))
      {
        current_col = 2;
      }
      else if( is_done( line[lexstart] ))
      {
        current_col = 5;
      }
      else
...
void readLine()
{
  ...
  if( mac_ptr )                 /* Inside macro? */
  {
    ...
    do
    {
      ...
      if( is_lower( mc ))        /* Encoded argument number?                   */
      {
...
void printLine( char *line, WORD32 loc, WORD32 val, LINESTYLE_T linestyle )
{
  ...
  case LINE_VAL:
    if( !listed )
    {
	fprintf( listfile, "%5d       %4.4o  ", lineno, (unsigned)val );
      fputs( line, listfile );
      listed = TRUE;
    }
    else
  ...
  case LINE_LOC_VAL:
    if( !listed )
    {
      ...
      {
	  fprintf( listfile, "%5d %5.5o %4.4o@ ", lineno, (unsigned)loc, (unsigned)val );
      }
      else
      {
	  fprintf( listfile, "%5d %5.5o %4.4o  ", lineno, (unsigned)loc, (unsigned)val );
    ...
    {
	fprintf( listfile, "      %5.5o %4.4o\n", (unsigned)loc, (unsigned)val );
    }
    /* update memory map */
    memory_map[loc] = TRUE;
    if (loc > memory_max) memory_max = loc;
    if (loc < memory_min) memory_min = loc;
    break;
  ...
  for( ix = 0; ix < symbol_top; ix++ )
  {
    if( M_MRI( symtab[ix].type ))
    {
      fprintf( permfile, "%-7s %s=%4.4o\n",
	       s_type, symtab[ix].name, (unsigned)symtab[ix].val );
    }
  }
...
void printCrossReference()
{
  ...
  for( ix = symbol_base; ix < symbol_top; ix++ )
  {
    ...
    case REDEFINED:
      fprintf( listfile, " M  %5ld  ", xreftab[xc_index] );
      break;

    default:
      fprintf( listfile, " A  %5ld  ", xreftab[xc_index] );
      break;
    }
    ...
    for( xc_cols = 0, xc = 1; xc < xc_refcount + 1; xc++, xc_cols++ )
    {
      ...
      fprintf( listfile, "  %5ld", xreftab[xc_index + xc] );
    }
    fprintf( listfile, "\n" );
  }
} /* printCrossReference()                                                    */
...
/*                                                                            */
/*  Synopsis:  Output a memory map for the file being assembled.              */
/*                                                                            */
/******************************************************************************/
void printMemoryMap()
{
  ...
  /* loop from loc 0 to maximum address used */
  for( loc = 0; loc <= memory_max; )
  {

    /* force a new page halfway thru the field */
    if( (loc & 03777) == 00000 ) page_lineno = LIST_LINES_PER_PAGE;
    if( page_lineno >= LIST_LINES_PER_PAGE )
    {
      topOfForm( list_title, s_memmap );
    }

    /* print two halves of a page over two lines */
    for( i = 0; i < 2; ++i )
    {
      page_lineno++;
      list_lineno++;
      fprintf( listfile, "%5d %05o/", list_lineno, (unsigned)loc );
      /* print 8 groups per line */
      for( j = 0; j < 8; ++j )
      {
	fprintf( listfile, " " );
	/* print 8 words per group */
	for( k = 0; k < 8; ++k )
	{
	  fprintf( listfile, "%c", memory_map[loc++] ? 'X' : '.');
	}
      }
      fprintf( listfile, "\n" );
    }
...
int copyMacLine( int length, int from, int term, int nargs )
{
  ...
  for( ix = from; ix < term; )
  {
    if( nargs && is_alpha( line[ix] ))      /* Start of symbol?                */
    {
      for( jx = ix + 1; jx < term; jx++)   /* Find end of symbol.             */
      {
        if( !is_alnum( line[jx] )) break;
      }
      ...
      if( kx >= nargs )
      {
        for ( ; ix < jx; )
        {
          mac_buffer[length++] = to_upper( line[ix++] );
		}
      }
    } /*end if nargs                                                          */
    else
...
void nextLexeme()
{
  ...
  if( is_alnum( line[cc] ))
  {
    while( is_alnum( line[cc] )) { cc++; }
  }
  else if( is_end( line[cc] ))
  {
...
BOOL pseudoOperators( PSEUDO_T val )
{
  ...
  case DEFINE:
    ...
    while(( line[lexstart] != '<' ) && ( !is_done( line[lexstart] )) &&
          ( count < MAC_MAX_ARGS ))
    {
      if ( !is_alpha( line[lexstart] ) && ( index == 0 ))
      {
        index = lexstart;
      }
    ...
    if( is_end( line[lexstart] ) || ( line[lexstart] == '/' ))
    {
      readLine();
      nextLexeme();
    }
  ...
  case ASCIZ:
    delim = line[lexstart];
    index = lexstart + 1;
    while( line[index] != delim && !is_end( line[index] ))
    {
      punchOutObject( clc, line[index]|0200 );
      incrementClc();
      index++;
    }
    if( (PSEUDO_T)val == ASCIZ )
    {
      punchOutObject( clc, 0 );
      incrementClc();
    }

    if( is_end( line[index] ))
    {
      cc = index;
      lexterm = cc;
  ...
  case DEVICE:
    ...
    count = 0;
    index = lexstart;
    while( !is_done( line[index] ) && !is_end( line[index] ))
    {
      pack = ( pack << 6 ) | ( line[index] & 077 );
  ...
  case TITLE:
    ...
    do
    {
      ...
      ix++;
    } while( line[ix] != delim && !is_end(line[ix]) );
...
void errorSymbol( EMSG_T *mesg, char *name, WORD32 col )
{
  ...
  if( pass == 2 )
  {
    s = ( name == NULL ) ? "" : name ;
    errors++;
    sprintf( linecol, "(%d:%ld)", lineno, col + 1 );
    fprintf( errorfile, "%s%-9s : error:  %s \"%s\" at Loc = %5.5o\n",
	     filename, linecol, mesg->file, s, (unsigned)clc );
...
void errorMessage( EMSG_T *mesg, WORD32 col )
{
  ...
  if( pass == 2 )
  {
    ...
    fprintf( errorfile, "%s%-9s : error:  %s at Loc = %5.5o\n",
	     filename, linecol, mesg->file, (unsigned)clc );
    saveError( mesg->list, col );
  }
  error_in_line = TRUE;
} /* errorMessage()                                                           */
